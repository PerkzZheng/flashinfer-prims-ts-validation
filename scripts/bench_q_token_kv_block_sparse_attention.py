...
"""Recorded sparse-attention CUDA driver; use run_sparse_attention_suite.py.

Derived from the qualified vLLM runner at b10972e13c. The validation repository
owns manifests and execution; FlashInfer and vLLM remain external dependencies.
Complete-pipeline timings include metadata/expansion and split-KV reduction.
"""
...
from sparse_attention_suites import SuiteCase, _expand_manifest, _select_cases
...
def _l2_flush_buffer() -> torch.Tensor:
    ...
    if buffer is None:
        ...
        buffer = torch.empty(flush_bytes, dtype=torch.uint8, device=device_index)
        # Uniform bytes can compress in Blackwell L2. A separate generator
        # preserves the benchmark tensors while initializing random scrub data.
        generator = torch.Generator(device=f"cuda:{device_index}")
        generator.manual_seed(0x5EED_C01D)
        buffer.random_(0, 256, generator=generator)
...
def _assert_metadata_equal(
    ...
    Memberships are byte-packed four per Int32. Compare each route's live
    bytes, not unused whole words beyond its prefix. FlashInfer's correctness
    suite separately checks that spare bytes in the final live word are zero.
    """
    ...
    *,
    ...
    result = {
        ...
        "trace": {
            ...
            "paths": [path.name for path in trace.source_paths],
...
def _new_result_document(
    ...
    vllm_root = Path(qsa.__file__).resolve().parents[5]
    validation_root = runner_path.parents[1]
    ...
    source = {
        ...
        "validation_git_state": _git_state(validation_root),
        "suite_helpers_sha256": _sha256(
            runner_path.with_name("sparse_attention_suites.py")
        ),
        "triton_attention_sha256": _sha256(Path(qsa.__file__)),
        "triton_indexer_sha256": _sha256(Path(qsa_indexer.__file__)),
    ...
    timing_contract = {
        "cuda_graph": True,
        "cold_l2": True,
        "l2_eviction_pattern": "seeded-random-uint8-add-one-v1",
        "l2_eviction_seed": 0x5EED_C01D,
        "l2_eviction_compressible_pattern": False,
        "l2_eviction_scope": "before-whole-graph-not-between-kernels",
        "l2_eviction_timed": False,
    ...
    resume_identity = {
        ...
        "cutlass_dsl": environment["cutlass_dsl"],
...
def main() -> int:
    ...
    for module, root in (
        (q_token_kv_block_sparse_metadata, args.source_root),
        (qsa, args.vllm_root),
        (qsa_indexer, args.vllm_root),
    ):
        if not Path(module.__file__).resolve().is_relative_to(root.resolve()):
            raise RuntimeError(
                f"{module.__name__} was not imported from the selected checkout"
            )
    manifest, cases = _expand_manifest(args.manifest, args.trace_root)
